...
The API is grouped into five delivery-ready epics. Each epic doc follows the same right-sized template — **Overview**, **Features → User Stories** (acceptance criteria + tasks), **Design Patterns** (applied vs. deferred), **Required Packages** (per-epic reference to the [canonical list](docs/epics/README.md#-packages)), **Testing** (Pest tests mapped to criteria + edge cases), and **Definition of Done**. Browse them in [`docs/epics`](docs/epics/README.md).
